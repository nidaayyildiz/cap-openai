...
from components.Openai.src.utils.response import build_structured_answering_response
from components.Openai.src.utils.prompt_builders import prepare_structured_prompt
from components.Openai.src.utils.openai_client import call_openai

class OpenaiStructuredAnswering(Component):
    """
    Answers questions by strictly following a predefined JSON structure.
    """
    def __init__(self, request, bootstrap):
        ...
        self.output_structure = configs.outputStructure.value
    ...
    def run(self):
        try:
            if not self.output_structure:
                raise ValueError("output_structure required")
                
            img = Image.get_frame(img=self.input_image, redis_db=self.redis_db)
            
            prompt_data = prepare_structured_prompt(self.output_structure, img.value)
